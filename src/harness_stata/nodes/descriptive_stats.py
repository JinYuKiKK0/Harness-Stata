...
_ITER_CAP = 6
_PREVIEW_ROWS = 3
_RTF_FILENAME = "01_descriptive_stats.rtf"
...
def _build_human_prompt(spec: EmpiricalSpec, merged: MergedDataset, rtf_path: Path) -> str:
    ...
    return (
        ...
        "`rtf_table_path` 已通过 `esttab using` 成功导出。\n"
...
async def descriptive_stats(state: WorkflowState) -> DescStatsReport:
    """对 MergedDataset 跑描述性统计,产出 do/log/rtf 路径与文字总结。"""
    err = _validate(state)
    if err is not None:
    ...
    workspace = resolve_stata_workspace("descriptive_stats")
    ...
    payload, do_path, log_path = await run_stata_agent(
        ...
        human_message=_build_human_prompt(spec, merged, rtf_path),
