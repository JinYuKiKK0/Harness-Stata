...
Lean MCP server for CSMAR metadata enumeration, probe validation, and local materialization for agent workflows.

Discovery is intentionally deterministic — the server exposes no search tools so that upstream CSMAR rate limits are never burned on server-side brute-force scans. The LLM is expected to narrow candidates itself using the list / schema outputs.
